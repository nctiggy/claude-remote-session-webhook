...
- [x] T019 [US3] Assert the fail-closed path end to end in `internal/httpapi/browser_test.go`: with the key set unobtainable, every browser request is refused rather than admitted (FR-009, SC-013), and the daemon does not crash or hang waiting on it.
...
- [x] T021a Serve the embedded static assets through the browser door in `internal/httpapi/server.go` and `internal/httpapi/render.go`: `GET /static/{path}` with correct Content-Type, `Cache-Control`, and the security headers every browser response carries. **Nothing serves `web/static/crswd.css` today** — T015 wrote it and T002 embedded it, but no task registered a route, so every browser renders the dashboard unstyled and `docs/design-system.md` is inert in practice (FR-023, FR-025). Expose **only** the `web/static/` subtree; the template tree must not be reachable through it. Also write `web/static/crswd.js` carrying the shared digital-rain loop — one `requestAnimationFrame` across every `.rain` canvas at ~14fps, wiped with a translucent fill, removed entirely under `prefers-reduced-motion` — because the CSP forbids inline script (FR-026), so the rain cannot live in a template. Test in `internal/httpapi/render_test.go`: the stylesheet is reachable with the right Content-Type, a `templates/` path is refused, and both are behind layer 1.
- [x] T021b Implement `GET /sessions/{id}/view` in `internal/httpapi/dashboard.go` — the single-session page every card already links to (`web/templates/partials/session-card.html` line 26 emits `/sessions/{{ .ID }}/view`). **Without it every card on the dashboard is a dead link.** Resolve ownership through `Manager.View` so that opening the page does not advance the idle clock (FR-034f), and render the card header plus the pane partial showing the session's current screen. US2's stream attaches to this page, so it must exist before T026. Test in `internal/httpapi/dashboard_test.go`: a session the viewer owns renders with its pane; another owner's session returns the uniform 404 byte-identical to an unknown id; and opening the page leaves `LastActivity` untouched.
- [x] T021c Extend the leak suite in `internal/audit/leak_test.go` to the **browser door**. It drives all six API routes with canary values and asserts none reaches a record or a log line — but it never drives the dashboard, so nothing currently proves the browser door does not leak pane content, a verified email, or a raw assertion into the trail (FR-035, FR-042, SC-008). Add `GET /`, `GET /sessions/{id}/view` and the static route to the same corpus, with an assertion carrying a canary email, and assert the same zero occurrences.
...
- [x] T022 [US2] Implement the SSE response in `internal/httpapi/stream.go`, clearing the write deadline **per response** with `http.NewResponseController(w).SetWriteDeadline(time.Time{})` ([research D3](./research.md)). **Do not set `WriteTimeout: 0`** — that removes the timeout from the six routes milestone 1 shipped with it. `internal/httpapi/server.go:40` carries the note this answers. Test that the stream survives past the server's write timeout while an ordinary route still times out.
- [x] T023 [US2] Implement the ordered open sequence from [contracts/stream.md](./contracts/stream.md) in `internal/httpapi/stream.go`: validated **identity** assertion (a service-token assertion is refused here as everywhere on this door) → cross-site refusal when `Sec-Fetch-Site` is present and not `same-origin`, treated as *present-and-wrong means refuse* never *absent means refuse* → ownership through the non-touching read from T012 → capacity **last**, so an unauthorised caller can never observe the cap's state. Count and admit in one critical section, closing the same race `Store.AddCapped` closes. Test each step's refusal in `internal/httpapi/stream_test.go`.
- [x] T024 [US2] Implement the capture loop in `internal/httpapi/stream.go`: capture on a 1s tick and emit **only when the screen differs from the last one sent** ([research D5](./research.md)) — an idle session would otherwise push an identical screen every second to every open tab forever. Emit a comment heartbeat on suppressed ticks so an intermediary does not time the connection out. Test with the fake controller that identical captures produce no event and a changed capture produces exactly one.
- [x] T025 [US2] Frame each event in `internal/httpapi/stream.go` as **one JSON string holding the whole screen** ([research D4](./research.md)). SSE's wire format is line-oriented, so a raw newline would start a new field; JSON encoding makes the framing independent of the content and guarantees the client receives a string. Test that a screen containing newlines, a lone `\r`, `<script>`, and quotes arrives intact and re-parses to the identical bytes.
- [x] T026 [US2] Build the pane in `web/templates/partials/pane.html` and `web/static/crswd.js`: `pane.textContent = JSON.parse(e.data)` — **the screen is replaced, not appended**. `docs/components.md` was corrected twice for this, once for the XSS (an htmx swap inserts payloads as markup) and once for the framing. Updating must never move the reader's scroll position, since a repainting screen has no bottom to follow (FR-032). Test that output containing markup renders as visible text with zero execution (SC-004).
...
- [x] T028 [US2] Enforce the stream lifecycle in `internal/httpapi/stream.go` (FR-034b, FR-034f): re-evaluate authorisation rather than establishing it once, stop delivering when the session ends or expires and say so in the view, **never advance the idle clock**, and never delay session teardown or daemon shutdown. Watching is not driving — a stream that postponed the idle deadline would let a forgotten browser tab hold an unsandboxed shell open indefinitely. Test that a reaped session ends its stream and that shutdown with open streams completes within the budget.
- [x] T029 [US2] Add the US2 acceptance suite in `internal/httpapi/stream_test.go` covering [contracts/stream.md](./contracts/stream.md)'s test table: the cap refuses past `CRSW_MAX_STREAMS`, two tabs on one session both work, a vanished browser is cleaned up, session output appears in **zero** audit records or log lines (FR-035), and the cross-site refusal fires.

- [x] T035 [US1] Make the fleet refresh itself (FR-017a–d, SC-006a). Added after the milestone closed, because what was missing was the requirement rather than the work: the page was rendered once and then described the host forever after, so a session created or reaped since the load stayed invisible until a manual reload (#15) — and every test passed while that was true, each one rendering a fresh page. The daemon chooses the interval, the fleet carries it as `data-refresh`, and `crswd.js` reloads on it. Not a partial swap: this milestone ships no htmx, and putting markup on the page from a script is what `docs/security.md` forbids on this client half ([contracts/dashboard.md](./contracts/dashboard.md)). A hidden tab refreshes when it comes back rather than on the timer (FR-017d). Test that the fleet carries the daemon's own interval and that the session page carries none — a reload there would tear down the stream already updating it.
...
- [x] T030 [P] Amend `docs/auth-and-sessions.md` — it is binding and this milestone makes it wrong in three ways. It opens "There are no browser sessions and no human login form" (this milestone creates both); its layer-1 code sample uses a JWT library, contradicting the zero-dependency decision ([research D1](./research.md)); and its two-door table has no service token. Add the stream-authorisation rule, which currently exists only in the spec.
