...
- [ ] T021c Extend the leak suite in `internal/audit/leak_test.go` to the **browser door**. It drives all six API routes with canary values and asserts none reaches a record or a log line — but it never drives the dashboard, so nothing currently proves the browser door does not leak pane content, a verified email, or a raw assertion into the trail (FR-035, FR-042, SC-008). Add `GET /`, `GET /sessions/{id}/view` and the static route to the same corpus, with an assertion carrying a canary email, and assert the same zero occurrences.
