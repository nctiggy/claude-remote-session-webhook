...
- [x] T041 Fill in `deploy/crswd.example.service` and `deploy/cloudflared.example.yml` with the secret sourced from 1Password rather than any repo file, and complete the deployment section of `README.md` including `journalctl --user -u crswd` for reading the audit trail.
- [x] T042 Run the full [quickstart.md](./quickstart.md) validation end to end against a real build and real tmux, and record the outcome in `ralph/PROGRESS.md`. Any deviation from the documented expectations is a defect in the code or the doc — fix one of them, do not paper over it.
