# Milestone 1 — completed task list (archived)

Kept rather than deleted. Forty-three iterations of `ralph/loop.sh` worked this list;
`ralph/PROGRESS.md` records what each one did and learned, and its entries reference
these T-numbers. Deleting the list would leave that notebook pointing at nothing.

Milestone 1 shipped, was reviewed cross-model, and is **deployed**: `crswd` runs as a
systemd user service behind a Cloudflare Tunnel with an Access application in front.
The design artifacts are in `specs/001-crswd-daemon-core/`.

Two defects found *after* this list was fully ticked are worth carrying forward, because
both were "documented, tested, and never wired":

- `session.NewReaper` had no caller outside tests. Every bound Principle VI calls
  non-negotiable — idle timeout, absolute lifetime — was implemented and switched off.
- `Store.Touch` had no caller outside tests, so the idle clock never moved.

A task that says "implement X" is not done when X exists. It is done when something
calls it.
...
- [x] T030 `internal/httpapi/isolation_test.go`: cross-session isolation suite — session A's output unreachable through any request scoped to B, and B's token on A's ID returning a 404 byte-identical to the unknown-ID response
...
- [x] T033 `internal/session/manager.go`: concurrent-session cap, refusing past `CRSW_MAX_SESSIONS` with 429 rather than degrading the host. Test covers creates racing at the boundary
...
- [x] T039 `internal/audit/leak_test.go`: drive every operation with distinctive prompt text and pane content, then assert **zero** occurrences of prompt text, pane output, tokens, token hashes, the shared secret, or any full body across all records and log lines
...
- [x] T042 Run the full `specs/001-crswd-daemon-core/quickstart.md` validation end to end against a real build and real tmux; record the outcome in `PROGRESS.md`. Any deviation is a defect in the code or the doc — fix one, do not paper over it
