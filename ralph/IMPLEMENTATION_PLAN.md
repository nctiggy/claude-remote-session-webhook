...
- [x] **T011** Add `font-size: var(--fs-input)` for `.field-input` **and** `.setting-input` to the same coarse block.
...
- [x] **T017** Re-read `docs/mobile-open-questions.md` and confirm all three questions are **still UNANSWERED** with their fallbacks intact. **This task verifies; it does not answer.** If any has been ticked, un-tick it and record what happened in `PROGRESS.md`.
