...
## ⛔ THE LOOP IS FINISHED. EVERYTHING LEFT WAITS ON THE OPERATOR
...
example key that happens to be valid is a real key in the repository.

T014 and everything after it wait on the operator running `crswd keygen`, adding
`RELEASE_SIGNING_KEY`, and committing the public line to **both** copies. T020 and T021 were the
work to take meanwhile and both are done. **There is no unblocked task left.** A fresh iteration
should read the handover in `PROGRESS.md`, Iteration 14, confirm nothing has changed, and stop.
...
done. T013 is now built and halted at the key itself. **Everything still open is behind the
operator generating one.**
