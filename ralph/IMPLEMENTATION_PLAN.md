...
- [x] T012 `internal/session/manager.go`: a read that resolves ownership **without advancing the idle clock**, and amend `Resolve`'s comment claiming to be the only path from a request to a session — this milestone makes that false
- [x] T013 `web/templates/partials/`: header with the verified identity top-right, status pill, session card, empty state, rain canvas. The documented action rows are a **parameter that is absent here**, not deleted code milestone 3 must restore
...
- [x] T019 Fail-closed end to end: with the key set unobtainable every browser request is refused rather than admitted, and the daemon neither crashes nor hangs
...
- [x] T021a `internal/httpapi/server.go` + `render.go`: **serve the embedded static assets** through the browser door — `GET /static/{path}` with correct Content-Type, `Cache-Control`, and the same security headers. Nothing serves `web/static/crswd.css` today, so every browser renders the dashboard unstyled and the whole design system is inert. Expose **only** `web/static/`; the template tree must not be reachable. Also write `web/static/crswd.js` with the shared rain loop (one `requestAnimationFrame` across every `.rain` canvas, ~14fps, removed under `prefers-reduced-motion`) — the CSP forbids inline script, so the rain cannot live in a template. Test: the stylesheet is reachable and correctly typed, a template path is **not**, and both are behind layer 1
- [x] T021b `internal/httpapi/dashboard.go`: **`GET /sessions/{id}/view`**, the single-session page every card already links to (`session-card.html` line 26). Without it every card is a dead link. Owner-scoped through `Manager.View` so watching does not advance the idle clock, rendering the card header plus the pane partial with the session's current screen. This is also the page US2's stream attaches to, so it must exist before T026. Test: a session the viewer owns renders; another owner's returns the uniform 404, byte-identical to unknown
- [x] T021c `internal/audit/leak_test.go`: extend the leak suite to the **browser door**. It drives all six API routes and asserts no prompt, pane, token or secret reaches a record — but it never drives the dashboard, so nothing proves the browser door does not leak pane content, an email, or an assertion into the trail. Add the dashboard routes to the same corpus with the same canaries
...
- [x] T024 Capture on a 1s tick, emitting **only when the screen changed**, with a comment heartbeat on suppressed ticks. An idle session would otherwise push an identical screen every second to every open tab forever
- [x] T025 Frame each event as **one JSON string holding the whole screen**. SSE is line-oriented, so a raw newline would start a new field; encoding makes framing independent of content
...
- [x] T030 Amend `docs/auth-and-sessions.md`: it still says there are no browser sessions and no human login form, its layer-1 sample uses a JWT library, and its two-door table has no service token. Add the stream-authorisation rule, which currently exists only in the spec
