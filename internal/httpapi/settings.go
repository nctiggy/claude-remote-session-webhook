...
// unitFacts is what this page says about the systemd unit on this host: one
// sentence about where it stands, and — when a newer one is waiting — the file
// it is waiting in and the command that shows the difference.
//
// The three travel together in one value, taken once (unitFactsOf), for
// doorFacts' reason: composed separately they would be free to disagree, and the
// shape of that disagreement is a page saying "nothing newer is waiting" above
// the command to diff the thing that is.
//
...
// end. So this is the one sentence on this page that carries a value, and the
// value is a path this daemon composed from its own HOME and its own constants.
type unitFacts struct {
	// Sentence is which of the answers below applies. Empty renders nothing,
	// which is the zero value and the only state that says nothing at all.
	Sentence string

	// Waiting is the file a release's own unit was left in, and is empty unless
	// that file is really on this disk (updater.UnitReport.Offer).
	Waiting string

	// Compare is the command that shows what is in it and not in theirs.
	//
	// It is a string to be read and pasted, never one this daemon runs: nothing
	// in this package executes a shell, and the constitution's rule that no shell
	// string is ever constructed is about the daemon's own actions rather than
	// about what it prints for a person. It is quoted all the same, because a
	// home directory with a space in it is a command that silently diffs the
	// wrong thing.
	Compare string
}

// The whole vocabulary for what became of this host's unit (M15/T004), and the
// end of this daemon's silence about it.
//
...
// unitFactsOf turns what internal/updater found on this disk into what the page
// says, and it is handed the error as well as the report on purpose.
//
// A read that failed is a sentence of its own rather than a blank row. The zero
...
// could take must not be allowed to fall through to it, and updater.Report
// refuses rather than returning it for the same reason.
//
...
func unitFactsOf(report updater.UnitReport, err error) unitFacts {
	if err != nil {
...
func (s *Server) updatePanelFor(r *http.Request, operator *access.VerifiedOperator) *updatePanel {
	...
	if s.updates.unit != nil {
		panel.Unit = unitFactsOf(s.updates.unit.Report())
	}
