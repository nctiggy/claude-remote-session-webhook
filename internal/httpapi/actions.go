...
// replacing — the form is a plain post and this tree's one script swaps no
// response — so the operator navigated to a bare `<p>` with no page around it.
// The sentences are unchanged and now live in outcome.go's map, where the fleet
// renders them after the redirect lands, next to the fleet the destroy changed.
//
...
// could not verify means a live unsandboxed shell may have survived, and #42 is
// explicit that it must not be reduced to a line of prose beside "renamed".
...
func (s *Server) destroyFromBrowser(w http.ResponseWriter, r *http.Request) {
	...
	// before the teardown so the unverified outcome below names the one it could
	// not confirm.
...
// the fleet renders as more than a line of prose (#42).
//
...
// What a create answers is an outcome code, exactly as a destroy's is (#42).
// The four refusals are outcomeBadName, outcomeBadWorkDir, outcomeLimited and
// outcomeCreateFailed, and the success is outcomeCreated; their sentences are
// milestone 3's own, moved into outcome.go's map.
//
// The one that changed shape is the success. It used to be the new card itself,
// rendered here and appended to a fleet by a swap nothing performed — so an
// operator who started a session was shown one card on a blank page. The fleet
// they are returned to now draws that card from the same projection (cardOf)
// alongside every other, which is the appending the plan described, done by the
// page that owns the grid.
...
func (s *Server) createFromBrowser(w http.ResponseWriter, r *http.Request) {
	...
	// The fleet, which is where the new card is (#42).
	//
	// This used to render the card here and hand it back as a fragment, on the
	// plan's expectation that something would append it to the grid. Nothing did,
	// so a create that worked put the operator on a page holding one card and
	// nothing else — no header, no summary, no fleet, no create form. The redirect
	// sends them to the page that draws the grid, and that page renders this
	// session from the same projection (cardOf) as every other, so the first
	// appearance of a card is a card in a fleet rather than a card on its own.
	//
...
// What a rename answers is an outcome code (#42): outcomeRenamed, outcomeBadName
...
func (s *Server) renameFromBrowser(w http.ResponseWriter, r *http.Request) {
	...
	// The fleet, where the card now carries the new name (#42). It used to be the
	// re-rendered card, handed back as a fragment on the expectation that
	// something would put it where the old one was — and nothing did, so a rename
	// that worked put the operator on a page holding one card and no page around
	// it. The name is the only thing about the card that moved, and the fleet
	// draws it from the same projection this route used to call.
...
// What a compact answers is an outcome code (#42): outcomeCompacted, and
...
//
// Everything else is the failed delivery. A paste that failed is the only cause and
