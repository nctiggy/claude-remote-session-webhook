...
**One value the daemon states rather than carries:** `CLAUDE_CODE_OAUTH_401_WAIT_MS=60000`.
Every session on the host shares one Claude credential store and one *rotating*
refresh token, so when the access token expires they race to refresh it, one wins,
and each loser replays a token already spent — and is logged out. It arrives as
bursts, not as one session at a time. Claude Code ships that back-off already and
defaults it to 60s only for a remote session's child; a tmux pane is not one, so it
took the zero. Setting the variable in the daemon's own environment overrides this,
and `0` turns it off.
...
| `CRSW_SESSION_ENVIRONMENT` | no | empty | Comma-separated environment variable **names** a session receives on top of the base set (`HOME`, `PATH`, `SHELL`, `USER`, `LOGNAME`, `TERM`, `LANG`, `LC_*`, `XDG_RUNTIME_DIR`, `TMUX_TMPDIR`), and on top of the one value the daemon states rather than carries, `CLAUDE_CODE_OAUTH_401_WAIT_MS=60000`. A session never inherits this daemon's environment, so the shared secret and every `CRSW_` setting stay on this side; this is the escape hatch for the one more variable a workflow needs. Names, not values. Naming a secret or anything `CRSW_` refuses at startup |
