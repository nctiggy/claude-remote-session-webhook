...
- 2026-08-08 — Any interruption of the fleet stream revealed the stalled note for good, so a routine daemon restart told the operator to reload; the live half now re-fetches the fleet when the stream reconnects, replaces the grid and the counts in place, announces a fleet that changed size, and leaves the note only where that re-fetch failed. (#99)
