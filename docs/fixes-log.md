...
- 2026-08-07 — CI runs only the untagged commands, so the `quickstart` suite rotted against #63 and #72 unseen: three cases asserted an adopted card knows neither its name nor its directory and that a clean stop reaps the fleet, all of which the daemon deliberately stopped doing. The expectations now match the shipped contract. The `ci.yml` steps that would have caught it are still owed — an App token without `workflows` scope cannot land them. (#87)
