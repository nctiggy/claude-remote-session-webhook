...
- 2026-08-07 — `/settings` shipped with nothing linking to it, so typing the address was the only way in; the header now carries one dim link beside the operator's address — before it, so the top-right stays identity — and the test holds the header at two anchors so the next one cannot make it a nav bar. (#91)
