...
- 2026-08-07 — Nothing verified the daemon's dependencies, so a host without tmux started clean and failed on the first create with whatever exec said; startup now refuses without tmux and names the platform's install command, warns on a start command that is not installed, and reads the *configured* command rather than a fixed name. (#71)
