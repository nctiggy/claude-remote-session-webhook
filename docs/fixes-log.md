...
- 2026-08-07 — The published audit-trail read `journalctl --user -u crswd -o cat | jq .` selected the unit's whole cgroup, so systemd's own lifecycle lines aborted jq on any deployment that had ever restarted; every copy now filters by syslog identifier with `-t crswd`, the unit states `SyslogIdentifier=` so that identifier is promised rather than inferred, and the hardening block explains the utempter `pututline` line it also surfaces. (#88)
