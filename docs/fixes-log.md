...
- 2026-08-07 — The start-command probe resolves PATH in the daemon's process while the command is typed into a pane shell that loads the operator's profile, so a working deployment was warned on every start that its sessions "will fail"; the warning now names the PATH it read and whose PATH decides, and predicts nothing. (#96)
