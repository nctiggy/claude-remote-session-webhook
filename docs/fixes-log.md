...
- 2026-09-08 — Every `claude` on this host shares one credential store and one rotating refresh token, so an expiring access token was a race whose losers replayed a spent token and were logged out — in bursts, 3 in 98s and 7 in 13min, not one session at a time. Claude Code ships the back-off already and defaults it to 60s only when `CLAUDE_CODE_REMOTE_SESSION_ID` marks a remote session's child; a tmux pane is not one, so it took the zero. The earlier fix exported the variable from one of the three start commands on the host, leaving a session made by hand from the dashboard still racing and believed covered. Composition happens once for all three, so `sessionenv.go` now *states* `CLAUDE_CODE_OAUTH_401_WAIT_MS=60000` rather than only carrying one the daemon happened to have — the operator's own value still wins, and `0` is off. Found while writing it: defaults are appended after the allowlist runs, so nothing screens them, and a default spelled `CRSW_` would have reached an unsandboxed shell past every green test; `excluded()` is now applied to both routes. Verified by reading the variable out of a real pane, which is the assertion the start-command-shaped fix would have passed. (#154)
